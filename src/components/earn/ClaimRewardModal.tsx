  ...
  async function onClaimReward() {
    if (stakingContract && stakingInfo?.stakedAmount) {
      ...
      await stakingContract
        .withdraw(poolMap[stakingInfo.stakingRewardAddress], `0x0`, {
          gasLimit: 300000,
        })
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claim accumulated WFX rewards`,
          })
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            <TYPE.body fontSize={20}>Claiming {stakingInfo?.earnedAmount?.toSignificant(6)} WFX</TYPE.body>
          </AutoColumn>
        </LoadingView>
      )}
      {hash && (
