...
export default function StakingModal({ isOpen, onDismiss, stakingInfo, userLiquidityUnstaked }: StakingModalProps) {
  ...
  const [approval, approveCallback] = useApproveCallback(parsedAmount, chainId && MINICHEF_ADDRESS[chainId])
  ...
  async function onStake() {
    ...
    if (stakingContract && parsedAmount && deadline) {
      if (approval === ApprovalState.APPROVED) {
        await stakingContract.deposit(`0x${parsedAmount.quotient.toString(16)}`, { gasLimit: 350000 })
      } else if (signatureData) {
        stakingContract
          .depositWithPermit(
            poolMap[stakingInfo.stakingRewardAddress],
      ...
      {attempting && !hash && (
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)} FX-V2</TYPE.body>
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            <TYPE.body fontSize={20}>Deposited {parsedAmount?.toSignificant(4)} FX-V2</TYPE.body>
          </AutoColumn>
        </SubmittedView>
      )}
    </Modal>
