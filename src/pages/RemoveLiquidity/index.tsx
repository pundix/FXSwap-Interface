...
export default function RemoveLiquidity({
  ...
  async function onRemove() {
    ...
    if (approval === ApprovalState.APPROVED) {
      // removeLiquidityETH
      if (oneCurrencyIsETH) {
        methodNames = ['removeLiquidityFX', 'removeLiquidityFXSupportingFeeOnTransferTokens']
        args = [
    ...
    // we have a signature, use permit versions of remove liquidity
      ...
      if (oneCurrencyIsETH) {
        methodNames = ['removeLiquidityFXWithPermit', 'removeLiquidityFXWithPermitSupportingFeeOnTransferTokens']
        args = [
          currencyBIsETH ? tokenA.address : tokenB.address,
          liquidityAmount.quotient.toString(),
          amountsMin[currencyBIsETH ? Field.CURRENCY_A : Field.CURRENCY_B].toString(),
  ...
  function modalHeader() {
    return (
      <AutoColumn gap={'md'} style={{ marginTop: '20px' }}>
        ...
        <TYPE.italic fontSize={12} color={theme.text2} textAlign="left" padding={'12px 0 20px 0'}>
      ...
      <>
        <RowBetween>
          <Text color={theme.text2} fontWeight={500} fontSize={16}>
            {'FX ' + currencyA?.symbol + '/' + currencyB?.symbol} Burned
          </Text>
          <RowFixed>
  ...
  return (
    <>
                  ...
                  <AutoColumn gap="10px">
                    ...
                    {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                      <RowBetween style={{ justifyContent: 'flex-end' }}>
                        {oneCurrencyIsETH ? (
                          <StyledInternalLink
                            to={`/remove/${currencyA?.isEther ? WETH9[chainId].address : currencyIdA}/${
                          ...
                          <StyledInternalLink
                            to={`/remove/${
                              currencyA && currencyEquals(currencyA, WETH9[chainId]) ? 'FX' : currencyIdA
                            }/${currencyB && currencyEquals(currencyB, WETH9[chainId]) ? 'FX' : currencyIdB}`}
                          >
                            Receive FX
                          </StyledInternalLink>
                        ) : null}
      ...
      {pair ? (
        <AutoColumn style={{ minWidth: '20rem', width: '100%', maxWidth: '480px', marginTop: '1rem' }}>
